...
namespace Persistance.Identity.Stores
{
        ...
        where TUserClaim : IdentityUserClaim<TKey>, new()
    {
        ...
        public virtual async Task AddToRoleAsync(TUser user, string roleName)
        {
            ...
            var roleEntity = await _roleStore.DbEntitySet.SingleOrDefaultAsync(r => r.Name.ToUpper() == roleName.ToUpper()).ConfigureAwait(false);
            if (roleEntity == null)
            {
        ...
        public virtual async Task<bool> IsInRoleAsync(TUser user, string roleName)
        {
            ...
            var role = await _roleStore.DbEntitySet.SingleOrDefaultAsync(r => r.Name.ToUpper() == roleName.ToUpper()).ConfigureAwait(false);
            if (role != null)
            {
                var userId = user.Id;
                var roleId = role.Id;
